...
export function ContactPageClient({ siteSettings, profile }: ContactPageClientProps) {
    ...
    // Derived data
    // Try to find email in socials if not explicit (schema agnostic)
    const emailSocial = siteSettings?.socials?.find((s: any) => s.platform?.toLowerCase() === 'email' || s.label?.toLowerCase() === 'email' || s.url?.startsWith('mailto:'))
    const contactEmail = emailSocial?.label || (emailSocial?.url ? emailSocial.url.replace('mailto:', '') : 'Contact via Form')

    // Fallback location
    const location = "Remote / Worldwide" // This is a safe fallback for a digital freelancer
                    ...
                    <div className="space-y-6">
                        ...
                        >
                            ...
                            <div>
                                ...
                                <p className="text-lg font-semibold">{contactEmail}</p>
                            </div>
                        </motion.div>
